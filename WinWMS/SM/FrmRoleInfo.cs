...
namespace WinWMS.SM
{
    public partial class FrmRoleInfo : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageHelper.MsgErrorShow("角色名称不能为空！");
                txtRName.Focus();
                return;
            }
            ...
            if (fModel.FId == 0 || (!string.IsNullOrEmpty(oldName) && oldName != roleName))
            {
                ...
                {
                    MessageHelper.MsgErrorShow("角色名称已经存在！");
                    txtRName.Focus();
                    return;
                }
            }
            ...
            if (bl)
            {
                MessageHelper.Info($"{btnText}角色", $"角色：{roleName} 信息{btnText}成功！");
                //刷新列表页面数据
                fModel.ReLoad?.Invoke();
            }
            else
            {
                MessageHelper.MsgErrorShow($"角色：{roleName} 信息{btnText}失败！");
                return;
            }
        }
    }
